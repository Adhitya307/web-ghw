    ...
    <div class="container form-container">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0"><i class="fas fa-plus me-2"></i>Tambah Data Piezometer - Right Bank</h3>
            </div>
            <div class="card-body">
                <!-- Alert Container -->
                ...
                <!-- Form Tambah Data -->
                <form id="createForm" action="<?= base_url('right-piez/store') ?>" method="post">
                    <?= csrf_field() ?>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="btn btn-success btn-action" id="submitBtn">
                            <i class="fas fa-save me-1"></i> Simpan & Hitung Otomatis
                        </button>
                    </div>
                    ...
                    <div class="text-center mt-4 loading-spinner" id="loadingSpinner">
                        ...
                        <p class="mt-2">Menyimpan data dan melakukan perhitungan...</p>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fungsi untuk menampilkan notifikasi perhitungan
    function showCalculationNotification(message) {
        calculationMessage.textContent = message;
        calculationNotification.style.display = 'block';
    }
    ...
    function checkDuplicate() {
        ...
        if (tahun && periode && tanggal) {
            fetch('<?= base_url('right-piez/check-duplicate') ?>', {
                method: 'POST',
                headers: {
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch('<?= base_url('right-piez/store') ?>', {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
        ...
        .then(data => {
            if (data.success) {
                if (data.calculation_success) {
                    showAlert('Data berhasil disimpan dan perhitungan selesai! Mengalihkan...', 'success');
                    setTimeout(() => {
                        window.location.href = '<?= base_url('right-piez') ?>';
                    }, 2000);
                } else {
                    showAlert('Data berhasil disimpan tetapi perhitungan gagal. Mengalihkan...', 'warning');
                    setTimeout(() => {
                        window.location.href = '<?= base_url('right-piez') ?>';
                    }, 3000);
                }
        ...
        .finally(() => {
            ...
            submitBtn.innerHTML = '<i class="fas fa-save me-1"></i> Simpan & Hitung Otomatis';
