...
<head>
    ...
    <title><?= $title ?? 'Bubble Tilt Meter BT-2 - PT Indonesia Power' ?></title>
...
<div class="container-fluid py-4">
    ...
    <div class="table-header">
        <h2 class="table-title">
            <i class="fas fa-mountain me-2"></i>Bubble Tilt Meter (BTM) - BT-2
        </h2>

        <!-- Button Group -->
        <div class="btn-group mb-3" role="group">
            <a href="<?= base_url('btm') ?>" class="btn btn-outline-primary btn-bt">BT-1</a>
            <a href="<?= base_url('btm/bt2') ?>" class="btn btn-primary btn-bt">
                <i class="fas fa-table"></i> BT-2
            </a>
        ...
        <table class="data-table table table-bordered table-hover" id="exportTable">
            <thead>
                ...
                <tr>
                    ...
                    <!-- BACAAN BT2 -->
                    <th colspan="4" class="bg-reading">BACAAN BT-2</th>
            ...
            <tbody id="dataTableBody">
                ...
                <?php else: ?>
                    <?php foreach($pengukuran as $item): 
                        ...
                        // Ambil data untuk BT2
                        $Y_US = $scatter['bt2']['Y_US'] ?? null;
                        $X_TB = $scatter['bt2']['X_TB'] ?? null;
                        $Y_cum = $scatter['bt2']['Y_cum'] ?? null;
                        $X_cum = $scatter['bt2']['X_cum'] ?? null;
                    ?>
                    <tr data-pid="<?= $p['id_pengukuran'] ?>">
                        ...
                        <!-- BACAAN DATA BT2 -->
                        <?php 
                            $bt2Bacaan = $bacaan['bt2'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt2Bacaan['US_GP']) ? number_format($bt2Bacaan['US_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt2Bacaan['US_Arah'] ?? '-') ?></td>
                        <td class="number-cell"><?= isset($bt2Bacaan['TB_GP']) ? number_format($bt2Bacaan['TB_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt2Bacaan['TB_Arah'] ?? '-') ?></td>

                        <!-- PERHITUNGAN DATA BT2 -->
                        <?php 
                            $bt2Perhitungan = $perhitungan['bt2'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt2Perhitungan['A_sec']) ? number_format($bt2Perhitungan['A_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['sin_A_rad']) ? number_format($bt2Perhitungan['sin_A_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['B_sec']) ? number_format($bt2Perhitungan['B_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['sin_B_rad']) ? number_format($bt2Perhitungan['sin_B_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['sin_C_rad']) ? number_format($bt2Perhitungan['sin_C_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['sin_C_deg']) ? number_format($bt2Perhitungan['sin_C_deg'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['Cosa']) ? number_format($bt2Perhitungan['Cosa'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt2Perhitungan['a_rad']) ? number_format($bt2Perhitungan['a_rad'], 6) : '-' ?></td>
                        <td><?= esc($bt2Perhitungan['DMS'] ?? '-') ?></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('exportExcel').addEventListener('click', function() {
        ...
        setTimeout(() => {
            try {
                ...
                const wb = XLSX.utils.table_to_book(table, {sheet: "Data BT2"});
                
                const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
                const filename = `BT2_Data_Export_${timestamp}.xlsx`;
    ...
    document.getElementById('calculateAll').addEventListener('click', function() {
        if (confirm('Hitung ulang semua data BT2? Proses ini mungkin memakan waktu.')) {
            const btn = this;
            ...
            .then(data => {
                if (data.success) {
                    alert('Perhitungan BT2 berhasil!');
                    location.reload();
                } else {
