...
<head>
    ...
    <title>Edit Data BTM - PT Indonesia Power</title>
...
<body>
    ...
    <div class="container form-container">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Data Bubble Tilt Meter (BTM)</h3>
            </div>
            <div class="card-body">
                <!-- Alert Container -->
                ...
                <!-- Form Edit Data -->
                <form id="editForm" action="<?= base_url('btm/update/' . ($pengukuran['id_pengukuran'] ?? '')) ?>" method="post">
                    <?= csrf_field() ?>
                    
                    <!-- PERBAIKAN: Method override yang benar -->
                        ...
                        <div class="horizontal-readings">
                            <div class="reading-item">
                                ...
                                <input type="text" class="form-control numeric-input" id="bt<?= $i ?>_US_GP" name="bt<?= $i ?>_US_GP"
                                       ...
                                       placeholder="0.000" 
                                       value="<?= isset($bacaan['bt'.$i]['US_GP']) && $bacaan['bt'.$i]['US_GP'] !== null ? esc($bacaan['bt'.$i]['US_GP']) : '0' ?>">
                            </div>
                            
                            <div class="reading-item">
                                <label for="bt<?= $i ?>_US_Arah" class="form-label">US Arah</label>
                                <select class="form-select" id="bt<?= $i ?>_US_Arah" name="bt<?= $i ?>_US_Arah">
                                    <option value="U" <?= ($bacaan['bt'.$i]['US_Arah'] ?? 'U') == 'U' ? 'selected' : '' ?>>U</option>
                                    <option value="S" <?= ($bacaan['bt'.$i]['US_Arah'] ?? 'U') == 'S' ? 'selected' : '' ?>>S</option>
                                </select>
                            </div>
                            ...
                            <div class="reading-item">
                                ...
                                <input type="text" class="form-control numeric-input" id="bt<?= $i ?>_TB_GP" name="bt<?= $i ?>_TB_GP"
                                       pattern="-?[0-9]*\.?[0-9]*" title="Hanya angka, titik, dan minus di depan diperbolehkan"
                                       placeholder="0.000" 
                                       value="<?= isset($bacaan['bt'.$i]['TB_GP']) && $bacaan['bt'.$i]['TB_GP'] !== null ? esc($bacaan['bt'.$i]['TB_GP']) : '0' ?>">
                            </div>
                            
                            <div class="reading-item">
                                <label for="bt<?= $i ?>_TB_Arah" class="form-label">TB Arah</label>
                                <select class="form-select" id="bt<?= $i ?>_TB_Arah" name="bt<?= $i ?>_TB_Arah">
                                    <option value="T" <?= ($bacaan['bt'.$i]['TB_Arah'] ?? 'T') == 'T' ? 'selected' : '' ?>>T</option>
                                    <option value="B" <?= ($bacaan['bt'.$i]['TB_Arah'] ?? 'T') == 'B' ? 'selected' : '' ?>>B</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="text-center mt-4 loading-spinner" id="loadingSpinner">
                        ...
                        <p class="mt-2 text-muted">Mengupdate data BTM dan menghitung ulang...</p>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkDuplicate() {
            ...
            const currentId = '<?= $pengukuran['id_pengukuran'] ?? '' ?>';
            ...
            if (tahun && periode && tanggal) {
                fetch('<?= base_url('btm/check-duplicate') ?>', {
                    ...
                    body: `tahun=${tahun}&periode=${periode}&tanggal=${tanggal}&current_id=${currentId}&<?= csrf_token() ?>=<?= csrf_hash() ?>`
                })
        ...
        form.addEventListener('submit', function(e) {
            ...
            // Submit form secara normal (CodeIgniter akan handle method override)
            ...
            .then(data => {
                if (data && data.success) {
                    showAlert('Data BTM berhasil diupdate dan perhitungan dihitung ulang! Mengalihkan...', 'success');
                    setTimeout(() => {
                        window.location.href = data.redirect || '<?= base_url('btm') ?>';
                    }, 1500);
                } else if (data && !data.success) {
                    showAlert('Error: ' + (data.message || 'Terjadi kesalahan saat update data'), 'danger');
                }
            })
            .catch(error => {
