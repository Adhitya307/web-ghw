...
<head>
    ...
    <title><?= $title ?? 'Bubble Tilt Meter BT-6 - PT Indonesia Power' ?></title>
    ...
    <style>
        ...
        /* Hapus gradient dan gunakan warna biru standar */
        .btn-bt6 {
            /* Tidak perlu style khusus, gunakan class btn-primary saja */
        }
    </style>
</head>
<body>
...
<div class="container-fluid py-4">
    ...
    <div class="table-header">
        <h2 class="table-title">
            <i class="fas fa-mountain me-2"></i>Bubble Tilt Meter (BTM) - BT-6
        </h2>
        ...
        <div class="btn-group mb-3" role="group">
            ...
            <a href="<?= base_url('btm/bt6') ?>" class="btn btn-primary btn-bt">
                <i class="fas fa-table"></i> BT-6
            </a>
        ...
        <table class="data-table table table-bordered table-hover" id="exportTable">
            <thead>
                ...
                <tr>
                    ...
                    <!-- BACAAN BT6 -->
                    <th colspan="4" class="bg-reading">BACAAN BT-6</th>
            ...
            <tbody id="dataTableBody">
                ...
                <?php else: ?>
                    <?php foreach($pengukuran as $item): 
                        ...
                        // Ambil data untuk BT6
                        $Y_US = $scatter['bt6']['Y_US'] ?? null;
                        $X_TB = $scatter['bt6']['X_TB'] ?? null;
                        $Y_cum = $scatter['bt6']['Y_cum'] ?? null;
                        $X_cum = $scatter['bt6']['X_cum'] ?? null;
                    ?>
                    <tr data-pid="<?= $p['id_pengukuran'] ?>">
                        ...
                        <!-- BACAAN DATA BT6 -->
                        <?php 
                            $bt6Bacaan = $bacaan['bt6'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt6Bacaan['US_GP']) ? number_format($bt6Bacaan['US_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt6Bacaan['US_Arah'] ?? '-') ?></td>
                        <td class="number-cell"><?= isset($bt6Bacaan['TB_GP']) ? number_format($bt6Bacaan['TB_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt6Bacaan['TB_Arah'] ?? '-') ?></td>

                        <!-- PERHITUNGAN DATA BT6 -->
                        <?php 
                            $bt6Perhitungan = $perhitungan['bt6'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt6Perhitungan['A_sec']) ? number_format($bt6Perhitungan['A_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['sin_A_rad']) ? number_format($bt6Perhitungan['sin_A_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['B_sec']) ? number_format($bt6Perhitungan['B_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['sin_B_rad']) ? number_format($bt6Perhitungan['sin_B_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['sin_C_rad']) ? number_format($bt6Perhitungan['sin_C_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['sin_C_deg']) ? number_format($bt6Perhitungan['sin_C_deg'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['Cosa']) ? number_format($bt6Perhitungan['Cosa'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt6Perhitungan['a_rad']) ? number_format($bt6Perhitungan['a_rad'], 6) : '-' ?></td>
                        <td><?= esc($bt6Perhitungan['DMS'] ?? '-') ?></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('exportExcel').addEventListener('click', function() {
        ...
        setTimeout(() => {
            try {
                ...
                const wb = XLSX.utils.table_to_book(table, {sheet: "Data BT6"});
                
                const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
                const filename = `BT6_Data_Export_${timestamp}.xlsx`;
    ...
    document.getElementById('calculateAll').addEventListener('click', function() {
        if (confirm('Hitung ulang semua data BT6? Proses ini mungkin memakan waktu.')) {
            const btn = this;
            const originalText = btn.innerHTML;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Menghitung...';
            btn.disabled = true;

            fetch('<?= base_url('btm/calculate-all-bt6') ?>', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    alert('Perhitungan BT6 berhasil!');
                    location.reload();
                } else {
