...
<head>
    ...
    <title><?= $title ?? 'Bubble Tilt Meter BT-8 - PT Indonesia Power' ?></title>
...
<div class="container-fluid py-4">
    ...
    <div class="table-header">
        <h2 class="table-title">
            <i class="fas fa-mountain me-2"></i>Bubble Tilt Meter (BTM) - BT-8
        </h2>
        ...
        <table class="data-table table table-bordered table-hover" id="exportTable">
            <thead>
                ...
                <tr>
                    ...
                    <!-- BACAAN BT8 -->
                    <th colspan="4" class="bg-reading">BACAAN BT-8</th>
            ...
            <tbody id="dataTableBody">
                ...
                <?php else: ?>
                    <?php foreach($pengukuran as $item): 
                        ...
                        // Ambil data untuk BT8
                        $Y_US = $scatter['bt8']['Y_US'] ?? null;
                        $X_TB = $scatter['bt8']['X_TB'] ?? null;
                        $Y_cum = $scatter['bt8']['Y_cum'] ?? null;
                        $X_cum = $scatter['bt8']['X_cum'] ?? null;
                    ?>
                    <tr data-pid="<?= $p['id_pengukuran'] ?>">
                        ...
                        <!-- BACAAN DATA BT8 -->
                        <?php 
                            $bt8Bacaan = $bacaan['bt8'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt8Bacaan['US_GP']) ? number_format($bt8Bacaan['US_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt8Bacaan['US_Arah'] ?? '-') ?></td>
                        <td class="number-cell"><?= isset($bt8Bacaan['TB_GP']) ? number_format($bt8Bacaan['TB_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt8Bacaan['TB_Arah'] ?? '-') ?></td>

                        <!-- PERHITUNGAN DATA BT8 -->
                        <?php 
                            $bt8Perhitungan = $perhitungan['bt8'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt8Perhitungan['A_sec']) ? number_format($bt8Perhitungan['A_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['sin_A_rad']) ? number_format($bt8Perhitungan['sin_A_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['B_sec']) ? number_format($bt8Perhitungan['B_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['sin_B_rad']) ? number_format($bt8Perhitungan['sin_B_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['sin_C_rad']) ? number_format($bt8Perhitungan['sin_C_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['sin_C_deg']) ? number_format($bt8Perhitungan['sin_C_deg'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['Cosa']) ? number_format($bt8Perhitungan['Cosa'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt8Perhitungan['a_rad']) ? number_format($bt8Perhitungan['a_rad'], 6) : '-' ?></td>
                        <td><?= esc($bt8Perhitungan['DMS'] ?? '-') ?></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('exportExcel').addEventListener('click', function() {
        ...
        setTimeout(() => {
            try {
                ...
                const wb = XLSX.utils.table_to_book(table, {sheet: "Data BT8"});
                
                const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
                const filename = `BT8_Data_Export_${timestamp}.xlsx`;
    ...
    document.getElementById('calculateAll').addEventListener('click', function() {
        if (confirm('Hitung ulang semua data BT8? Proses ini mungkin memakan waktu.')) {
            const btn = this;
            const originalText = btn.innerHTML;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Menghitung...';
            btn.disabled = true;

            fetch('<?= base_url('btm/calculate-all-bt8') ?>', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    alert('Perhitungan BT8 berhasil!');
                    location.reload();
                } else {
