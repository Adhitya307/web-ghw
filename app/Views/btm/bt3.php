...
<head>
    ...
    <title><?= $title ?? 'Bubble Tilt Meter BT-3 - PT Indonesia Power' ?></title>
...
<div class="container-fluid py-4">
    ...
    <div class="table-header">
        <h2 class="table-title">
            <i class="fas fa-mountain me-2"></i>Bubble Tilt Meter (BTM) - BT-3
        </h2>
        ...
        <div class="btn-group mb-3" role="group">
            ...
            <a href="<?= base_url('btm/bt3') ?>" class="btn btn-primary btn-bt">
        ...
        <table class="data-table table table-bordered table-hover" id="exportTable">
            <thead>
                ...
                <tr>
                    ...
                    <!-- BACAAN BT3 -->
                    <th colspan="4" class="bg-reading">BACAAN BT-3</th>
            ...
            <tbody id="dataTableBody">
                ...
                <?php else: ?>
                    <?php foreach($pengukuran as $item): 
                        ...
                        // Ambil data untuk BT3
                        $Y_US = $scatter['bt3']['Y_US'] ?? null;
                        $X_TB = $scatter['bt3']['X_TB'] ?? null;
                        $Y_cum = $scatter['bt3']['Y_cum'] ?? null;
                        $X_cum = $scatter['bt3']['X_cum'] ?? null;
                    ?>
                    <tr data-pid="<?= $p['id_pengukuran'] ?>">
                        ...
                        <!-- BACAAN DATA BT3 -->
                        <?php 
                            $bt3Bacaan = $bacaan['bt3'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt3Bacaan['US_GP']) ? number_format($bt3Bacaan['US_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt3Bacaan['US_Arah'] ?? '-') ?></td>
                        <td class="number-cell"><?= isset($bt3Bacaan['TB_GP']) ? number_format($bt3Bacaan['TB_GP'], 2) : '-' ?></td>
                        <td><?= esc($bt3Bacaan['TB_Arah'] ?? '-') ?></td>

                        <!-- PERHITUNGAN DATA BT3 -->
                        <?php 
                            $bt3Perhitungan = $perhitungan['bt3'] ?? [];
                        ?>
                        <td class="number-cell"><?= isset($bt3Perhitungan['A_sec']) ? number_format($bt3Perhitungan['A_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['sin_A_rad']) ? number_format($bt3Perhitungan['sin_A_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['B_sec']) ? number_format($bt3Perhitungan['B_sec'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['sin_B_rad']) ? number_format($bt3Perhitungan['sin_B_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['sin_C_rad']) ? number_format($bt3Perhitungan['sin_C_rad'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['sin_C_deg']) ? number_format($bt3Perhitungan['sin_C_deg'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['Cosa']) ? number_format($bt3Perhitungan['Cosa'], 6) : '-' ?></td>
                        <td class="number-cell"><?= isset($bt3Perhitungan['a_rad']) ? number_format($bt3Perhitungan['a_rad'], 6) : '-' ?></td>
                        <td><?= esc($bt3Perhitungan['DMS'] ?? '-') ?></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('exportExcel').addEventListener('click', function() {
        ...
        setTimeout(() => {
            try {
                ...
                const wb = XLSX.utils.table_to_book(table, {sheet: "Data BT3"});
                
                const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '');
                const filename = `BT3_Data_Export_${timestamp}.xlsx`;
    ...
    document.getElementById('calculateAll').addEventListener('click', function() {
        if (confirm('Hitung ulang semua data BT3? Proses ini mungkin memakan waktu.')) {
            const btn = this;
            ...
            .then(data => {
                if (data.success) {
                    alert('Perhitungan BT3 berhasil!');
                    location.reload();
                } else {
