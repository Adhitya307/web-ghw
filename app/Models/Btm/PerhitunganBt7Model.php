...
class PerhitunganBt7Model extends Model
{
    protected $DBGroup          = 'btm';
    protected $table            = 'p_bt_7';
    ...
    // Hitung rumus p_bt_7 berdasarkan data bacaan
    public function hitungRumusBt7($id_pengukuran_sekarang, $bacaanBt7Model, $bacaanBt7Sebelumnya = null)
    {
        // Ambil data bacaan sekarang
        $bacaan_sekarang = $bacaanBt7Model->getByPengukuran($id_pengukuran_sekarang);
        
        if (!$bacaan_sekarang) {
            throw new \Exception('Data bacaan BT7 tidak ditemukan');
        }

        // Jika tidak ada data sebelumnya, gunakan nilai default
        if (!$bacaanBt7Sebelumnya) {
            return $this->hitungDenganDefault($bacaan_sekarang);
        }

        // =====================
        // RUMUS SESUAI EXCEL UNTUK BT7
        // =====================

        // 1. A_sec = SQRT((US_GP_sekarang - US_GP_sebelumnya)^2) * 1.14375
        // BEDA: *1.14375 (BT1: *1.14329, BT2: *1.14329, BT3: *1.14373, BT4: *1.14329, BT6: *1.1442)
        $selisih_US = $bacaan_sekarang['US_GP'] - $bacaanBt7Sebelumnya['US_GP'];
        ...
        // 3. B_sec = SQRT((TB_GP_sekarang - TB_GP_sebelumnya)^2) * 1.14438
        // BEDA: *1.14438 (BT1: *1.14466, BT2: *1.1442, BT3: *1.14375, BT4: *1.14375, BT6: *1.14466)
        $selisih_TB = $bacaan_sekarang['TB_GP'] - $bacaanBt7Sebelumnya['TB_GP'];
        $B_sec = sqrt(pow($selisih_TB, 2)) * 1.144375;
    ...
    private function hitungDenganDefault($bacaan_sekarang)
    {
        ...
        // BEDA: *1.14375
        $A_sec = $bacaan_sekarang['US_GP'] * 1.14375;
        $A_dalam_derajat = $A_sec * (1/3600);
        $A_dalam_radian = deg2rad($A_dalam_derajat);
        $sin_A_rad = sin($A_dalam_radian);
        
        // BEDA: *1.14438
        $B_sec = $bacaan_sekarang['TB_GP'] * 1.14438;
        $B_dalam_derajat = $B_sec * (1/3600);
        $B_dalam_radian = deg2rad($B_dalam_derajat);
        $sin_B_rad = sin($B_dalam_radian);
    ...
    // Simpan hasil perhitungan BT7
    public function simpanPerhitunganBt7($data)
    {
        return $this->insert($data);
    }

    // Update hasil perhitungan BT7
    public function updatePerhitunganBt7($id_perhitungan, $data)
    {
        return $this->update($id_perhitungan, $data);
    }

    // Hapus hasil perhitungan BT7
    public function hapusPerhitunganBt7($id_perhitungan)
    {
        return $this->delete($id_perhitungan);
    }

    // Get semua perhitungan BT7
    public function getAllPerhitunganBt7()
    {
