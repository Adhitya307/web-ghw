...
class PerhitunganBt6Model extends Model
{
    protected $DBGroup          = 'btm';
    protected $table            = 'p_bt_6';
    ...
    // Hitung rumus p_bt_6 berdasarkan data bacaan
    public function hitungRumusBt6($id_pengukuran_sekarang, $bacaanBt6Model, $bacaanBt6Sebelumnya = null)
    {
        // Ambil data bacaan sekarang
        $bacaan_sekarang = $bacaanBt6Model->getByPengukuran($id_pengukuran_sekarang);
        
        if (!$bacaan_sekarang) {
            throw new \Exception('Data bacaan BT6 tidak ditemukan');
        }

        // Jika tidak ada data sebelumnya, gunakan nilai default
        if (!$bacaanBt6Sebelumnya) {
            return $this->hitungDenganDefault($bacaan_sekarang);
        }

        // =====================
        // RUMUS SESUAI EXCEL UNTUK BT6
        // =====================

        // 1. A_sec = SQRT((US_GP_sekarang - US_GP_sebelumnya)^2) * 1.1442
        // BEDA: *1.1442 (BT1: *1.14329, BT2: *1.14329, BT3: *1.14373, BT4: *1.14329)
        $selisih_US = $bacaan_sekarang['US_GP'] - $bacaanBt6Sebelumnya['US_GP'];
        $A_sec = sqrt(pow($selisih_US, 2)) * 1.1442;
        ...
        // BEDA: *1.14466 (BT1: *1.14466, BT2: *1.1442, BT3: *1.14375, BT4: *1.14375)
        $selisih_TB = $bacaan_sekarang['TB_GP'] - $bacaanBt6Sebelumnya['TB_GP'];
    ...
    private function hitungDenganDefault($bacaan_sekarang)
    {
        ...
        // BEDA: *1.1442
        $A_sec = $bacaan_sekarang['US_GP'] * 1.1442;
        $A_dalam_derajat = $A_sec * (1/3600);
        $A_dalam_radian = deg2rad($A_dalam_derajat);
        $sin_A_rad = sin($A_dalam_radian);
        
        // BEDA: *1.14466
        $B_sec = $bacaan_sekarang['TB_GP'] * 1.14466;
        $B_dalam_derajat = $B_sec * (1/3600);
        $B_dalam_radian = deg2rad($B_dalam_derajat);
        $sin_B_rad = sin($B_dalam_radian);
    ...
    // Simpan hasil perhitungan BT6
    public function simpanPerhitunganBt6($data)
    {
        return $this->insert($data);
    }

    // Update hasil perhitungan BT6
    public function updatePerhitunganBt6($id_perhitungan, $data)
    {
        return $this->update($id_perhitungan, $data);
    }

    // Hapus hasil perhitungan BT6
    public function hapusPerhitunganBt6($id_perhitungan)
    {
        return $this->delete($id_perhitungan);
    }

    // Get semua perhitungan BT6
    public function getAllPerhitunganBt6()
    {
