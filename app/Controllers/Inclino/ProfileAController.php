...
class ProfileAController extends BaseController
{
    ...
    {
        // Get available years from profil_a table
        $years = $this->getAvailableYears();
        
        $data = [
            'title' => 'Profil A Monitoring - PT Indonesia Power',
            'years' => $years
        ];
        
        return view('inclino/profile_a_view', $data);
    }
    
    /**
     * Get data Profil A berdasarkan tahun - AJAX
     */
    public function getDataByYear()
    {
        ...
        try {
            $data = $this->getProfilADataByYear($year);
    ...
    /**
     * Export Profil A to Excel - Download langsung
     */
    public function exportToExcel()
    {
        ...
        try {
            ...
            // Set judul utama
            $sheet->setCellValue('A1', 'PROFIL A DATA - PT INDONESIA POWER');
            ...
            // Generate filename
            $filename = 'Profil_A_Data_' . ($year ?: 'All') . '_' . date('Ymd_His') . '.xlsx';
    ...
    /**
     * Get Profil A data by year in pivot format
     */
    private function getProfilADataByYear($year)
    {
        try {
            $builder = $this->profilAModel->db->table('profil_a');
            
            // Base query - ambil semua data untuk tahun yang dipilih
            $builder->select('depth, reading_date, nilai_profil_a')
                    ->orderBy('depth', 'ASC')
            ...
            // Sort depths dari 0.5 ke 80.0 (bukan sebaliknya)
            ...
            log_message('error', 'getProfilADataByYear Error: ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    {
        try {
            $query = $this->profilAModel->db->query("
