...
class L7L9SheetController extends BaseSheetController
{
    /**
     * Create Sheet untuk Grafik History L7-L9 dengan struktur sama persis seperti L1-L3
     */
    public function createGrafikHistoryL7L9Sheet($sheet, $pengukuranData, $tahunFilter, $periodeFilter, $dmaFilter)
    {
        // ===== SET PAGE SETUP =====
        $sheet->getPageSetup()
        ...
        // L-7 Ambang Batas - SAMA DENGAN L1-L3
        ...
        // L-8 Ambang Batas - SAMA DENGAN L1-L3
        ...
        // L-9 Ambang Batas - SAMA DENGAN L1-L3
        ...
        if (empty($pengukuranData)) {
            ...
            foreach ($pengukuranData as $index => $p) {
                ...
                $sheet->setCellValue('D' . $currentRow, '652.06');
                $sheet->setCellValue('E' . $currentRow, '652.96');
                $sheet->setCellValue('F' . $currentRow, '655.96');
                
                $status_L07 = $this->getStatusByType($t_psmetrik_L07, 'L07');
                ...
                $sheet->setCellValue('J' . $currentRow, '657.84');
                $sheet->setCellValue('K' . $currentRow, '658.74');
                $sheet->setCellValue('L' . $currentRow, '661.74');
                
                $status_L08 = $this->getStatusByType($t_psmetrik_L08, 'L08');
                ...
                $sheet->setCellValue('P' . $currentRow, '621.15');
                $sheet->setCellValue('Q' . $currentRow, '622.05');
                $sheet->setCellValue('R' . $currentRow, '625.05');
                
                $status_L09 = $this->getStatusByType($t_psmetrik_L09, 'L09');
            ...
            if ($currentRow > $startDataRow) {
                ...
                foreach ($statusData as $row => $statuses) {
                    $cellG = 'G' . $row;
                    $statusL7 = $statuses['L07'];
                    $this->applyStatusColorToCell($sheet, $cellG, $statusL7, $sheet->getCell($cellG)->getValue());
                    
                    $cellM = 'M' . $row;
                    $statusL8 = $statuses['L08'];
                    $this->applyStatusColorToCell($sheet, $cellM, $statusL8, $sheet->getCell($cellM)->getValue());
                    
                    $cellS = 'S' . $row;
                    $statusL9 = $statuses['L09'];
                    $this->applyStatusColorToCell($sheet, $cellS, $statusL9, $sheet->getCell($cellS)->getValue());
                }
    ...
    /**
     * Fill Ambang Batas Data L7 - DIUBAH MIRIP DENGAN L1-L3
     */
    private function fillAmbangBatasDataL7($sheet, $startCell, $endCell)
    {
        ...
        $data = [
            ['Peringatan', '≥652.96', '', ''],  // Baris 7
            ['Bahaya', '≥655.96', '', ''],      // Baris 8
            ['T.Psmetrik', '≤652.95', '≤655.95', '≥655.96']  // Baris 9
        ];
    ...
    /**
     * Fill Ambang Batas Data L8 - DIUBAH MIRIP DENGAN L1-L3
     */
    private function fillAmbangBatasDataL8($sheet, $startCell, $endCell)
    {
        ...
        $data = [
            ['Peringatan', '≥658.74', '', ''],
            ['Bahaya', '≥661.74', '', ''],
            ['T.Psmetrik', '≤658.73', '≤661.73', '≥661.74']
        ];
    ...
    /**
     * Fill Ambang Batas Data L9 - DIUBAH MIRIP DENGAN L1-L3
     */
    private function fillAmbangBatasDataL9($sheet, $startCell, $endCell)
    {
        ...
        $data = [
            ['Peringatan', '≥622.05', '', ''],
            ['Bahaya', '≥625.05', '', ''],
            ['T.Psmetrik', '≤622.04', '≤625.04', '≥625.05']
        ];
